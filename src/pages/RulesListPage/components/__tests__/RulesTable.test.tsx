...
import { RulesListPage } from '@/pages/RulesListPage/RulesListPage.tsx'
...
describe('RulesListPage', () => {
  ...
  describe('Page States', () => {
    ...
    describe('empty', () => {
      it('shows empty message when no rules exist', async () => {
        ...
        renderPage()
        await waitFor(() => expectVisible(/no rules found/i), { timeout: 10000 })
      })
    })
  ...
  describe('Bulk Actions', () => {
    ...
    describe('Delete Modal', () => {
      ...
      it('shows correct rule count', async () => {
        await selectAllRules()
        await waitFor(() => expectVisible(/rules selected/i), { timeout: 10000 })
        await clickDeleteButton()
        await waitFor(() => expectVisible(/delete 3 rule/i), { timeout: 10000 })
      })
    ...
    describe('Move Modal', () => {
      it('opens when Move clicked', async () => {
        // Setup user with groups for this specific test
        ...
        // Need to reset and setup fresh since we need different user data
        server.use(createUserHandler(userWithGroups), createRulesHandler(testRules))
        await new Promise((resolve) => setTimeout(resolve, 0))

        // Re-render is needed because beforeEach already rendered with testUser
        // Using screen queries that will find elements from the already-rendered page
        await selectRule(1)
  ...
  describe('Table Display', () => {
    ...
    it('displays rule count in sidebar', async () => {
      ...
      renderPage()
      await waitFor(() => expectVisible('7'), { timeout: 10000 })
    })
  })
